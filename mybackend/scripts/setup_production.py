...
class ProductionSetup:
    ...
    def find_latest_export_files(self):
        ...
        for pattern in ['challenge_categories_*.json', 'challenges_*.json', 
                       'admin_users_*.json', 'site_settings_*.json']:
            files = list(self.data_dir.glob(pattern))
            if files:
                # Get the most recent file
                latest_file = max(files, key=lambda f: f.stat().st_mtime)
                key = pattern.split('_')[0] + '_' + pattern.split('_')[1] if len(pattern.split('_')) > 1 else pattern.split('_')[0]
...
def main():
    ...
    if not export_files:
        ...
        # Create admin user
        admin_creds_file = setup.data_dir / "admin_credentials_*.json"
        admin_files = list(setup.data_dir.glob("admin_credentials_*.json"))
        admin_file = admin_files[0] if admin_files else None
        
        setup.create_admin_user(admin_file)
    ...
    print("2. Verify challenge data is loaded")
