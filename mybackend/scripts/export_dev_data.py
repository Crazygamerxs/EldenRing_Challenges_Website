...
class DataExporter:
    ...
    def export_model_data(self, model_class, filename_prefix, exclude_fields=None):
    ...
    def export_core_data(self):
        ...
        if admin_users.exists():
            # Create a temporary queryset for admin users only
            core_exports['admin_users'] = self.export_model_data(
                type('AdminUser', (), {'objects': type('', (), {'all': lambda: admin_users})})(),
                'admin_users',
                exclude_fields=['password']  # We'll handle passwords separately
            )
    ...
    def export_optional_data(self):
        ...
        if regular_users.exists():
            optional_exports['users'] = self.export_model_data(
                type('RegularUser', (), {'objects': type('', (), {'all': lambda: regular_users})})(),
                'regular_users',
